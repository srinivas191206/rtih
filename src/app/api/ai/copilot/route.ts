...
export async function POST(request: Request) {
  try {
    ...
    if (role === 'Program Manager') {
      ...
      } else if (lowerPrompt.includes('grant') || lowerPrompt.includes('funding') || lowerPrompt.includes('seed') || lowerPrompt.includes('capital')) {
        text = `### 💰 Tailored Funding Options for ${startupName}\n\nBased on your venture profile, here are the matching grants you qualify for under the **Andhra Pradesh Startup Policy**:\n\n1. **Ratan Tata Seed Capital Fund**: Since you are in the *${stage}* stage, you qualify for up to **₹15 Lakhs** at 0% equity. This grant is routed through the central Amaravati hub.\n2. **AP Innovation Subsidies**: Up to **₹5 Lakhs** for testing and validating prototypes in regional spokes.\n3. **Women Entrepreneur Subsidies**: If your startup has >51% female equity, you qualify for an additional ₹5 Lakhs direct subsidy and a 50% discount on co-working space rentals.\n\n**Action Item:** Ensure you complete the *Customer Discovery* course and log 20+ interviews to unlock the manager's endorsement.`;
      } else if (lowerPrompt.includes('milestone') || lowerPrompt.includes('gps') || lowerPrompt.includes('learn') || lowerPrompt.includes('curriculum') || lowerPrompt.includes('course')) {
        text = `### 🗺️ Recommended Roadmap & Curriculum for ${startupName}\n\nSince **${startupName}** is currently in the **${stage}** stage, your primary learning and execution roadmap is centered on product-market fit.\n\n**Recommended Learning Modules:**\n* **For Idea & Validation Stages:** Complete the *Customer Discovery & Problem Validation* module and conduct 20+ user interviews.\n* **For Prototype & MVP Stages:** Focus on *Value Proposition Design* & *MVP Architecture* to build a functional prototype.\n* **For Scaling Stages:** Complete *Unit Economics & Pitching to Seed Investors*.\n\n**Next Step:** Complete the pending tasks assigned by your mentor (**${activeMentor}**) in the *Tasks* panel.`;
      } else if (lowerPrompt.includes('hire') || lowerPrompt.includes('team') || lowerPrompt.includes('intern') || lowerPrompt.includes('recruit')) {
        text = `### 👥 Student Talent Recruitment for ${startupName}\n\nTo bridge your engineering and operations talent gaps, you can recruit student interns directly from RTIH's university partner outposts near **${district}**:\n* **IIT Tirupati** (Deep Tech & Engineering)\n* **SRM University AP** (Software Engineering & AI)\n* **Andhra University** (Marine Science, Biotech, & General Management)\n\n**Action Item:** Go to the *Recruitment* panel to post a Job Listing. It will automatically sync with the placement cells of these partner universities.`;
      } else if (lowerPrompt.includes('mentor') || lowerPrompt.includes('coach') || lowerPrompt.includes('advisor') || lowerPrompt.includes('session')) {
        text = `### 🤝 Mentorship Alignment\n\nYour startup is matched with specialized mentors in the *${sector}* space. Your active mentor is **${activeMentor}**.\n\n**Recommendations:**\n1. Share your latest customer discovery interviews or prototype demo video with them via the *Documents* panel.\n2. Schedule a 30-minute review session using the *Mentors* tab.\n3. Use the *Communications* tab to chat with them directly.`;
      } else if (lowerPrompt.includes('risk') || lowerPrompt.includes('vulnerable') || lowerPrompt.includes('health') || lowerPrompt.includes('score')) {
        text = `### ⚠️ Venture Health & Risk Audit for ${startupName}\n\nYour health score is evaluated across milestone completions, runway, and mentor engagement:\n* **Milestone Progress:** Keep logging your customer discovery interviews to prevent milestone stagnation.\n* **Runway Protection:** Plan your expenditures. Accessing the Ratan Tata Seed Capital matching grant will provide 6+ months of runway.\n* **Engagement:** Reach out to **${activeMentor}** if you are stuck on product roadmap pivots.`;
      } else {
        text = `### 🚀 RTIH AI Startup Coach (${startupName})\n\nI am here to help you accelerate **${startupName}**. You can ask me:\n\n* \`What grants do I qualify for under the AP Startup Policy?\`\n* \`What learning modules should I complete next?\`\n* \`How do I hire engineering interns from local universities?\`\n* \`Review my milestone progress and suggest next tasks\``;
      }
    }
